...
const NFTCards: React.FC = () => {
  ...
  // Rendu d'une carte NFT
  const renderNFTCard = (nft: NFTConfig) => {
    const metadata = nftMetadata[nft.id];
    ...
    return (
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <div className="text-white/80 text-sm leading-relaxed mb-4 px-2">
            <div style={{
              display: '-webkit-box',
              WebkitLineClamp: 3,
          ...
          <div
            className={`mt-6 bg-black/40 backdrop-blur-sm rounded-lg overflow-hidden transition-all duration-500 ${
              activeDropdown === nft.id ? 'max-h-none p-6 border border-white/20' : 'max-h-0 p-0'
            }`}
          >
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 justify-items-center">
            {nftConfig.slice(0, 3).map(nft => renderNFTCard(nft))}
          </div>

          <div className="flex justify-center">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl">
              {nftConfig.slice(3, 5).map(nft => renderNFTCard(nft))}
            </div>
          </div>
        </div>
