...
const DashboardV2 = () => {
  ...
  // ✅ CORRIGÉ - Traiter les investissements et utiliser les données du smart contract
  useEffect(() => {
    const calculateAllReturns = async () => {
      ...
      try {
        const processed = await Promise.all(safeActiveInvestments.map(async (investment) => {
          ...
          // ✅ NOUVEAU - Utiliser getStakeDetails du smart contract
          try {
            const stakeDetails = await getStakeDetails(address, stakeId);
            const totalRewards = stakeDetails.totalRewards / 1e18; // Convertir wei vers USDC
            const baseRewards = stakeDetails.baseRewards / 1e18;
            const nftMultiplierFromContract = stakeDetails.nftMultiplier / 10000; // Convertir basis points
            
            // Calculer les récompenses quotidiennes depuis le smart contract
            const startDate = new Date(investment.startDate);
            const currentDate = new Date();
            const daysElapsed = Math.max(1, Math.floor((currentDate.getTime() - startDate.getTime()) / (1000 * 3600 * 24)));
            const dailyRewardsFromContract = totalRewards / daysElapsed;
            
            return { 
              investment: {
                ...investment,
                dailyReturn: dailyRewardsFromContract // ✅ Utiliser le calcul du smart contract
              }, 
              plan, 
              returns: totalRewards, // ✅ Directement du smart contract
              baseRewards: baseRewards,
              nftMultiplier: nftMultiplierFromContract
            };
          } catch (contractError) {
            console.warn('Erreur getStakeDetails, fallback vers calculateReturns:', contractError);
  ...
  }, [safeActiveInvestments, safePlans, calculateReturns, address]); // ✅ Retiré multiplier, ajouté address
