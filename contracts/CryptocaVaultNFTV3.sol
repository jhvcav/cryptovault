...
contract CryptocaVaultNFTV3 is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    // Mapping pour tracker l'eligibilite à la fidelite
    mapping(address => bool) public fidelityEligible;

    // Ajouter une mapping pour tracker les tokens par proprietaire
    ...
    event FidelityEligibilitySet(address indexed user, bool eligible);
    
    constructor(
        address _usdcToken
    ) ERC721("CryptocaVault Access NFT", "CVNFT") {
    ...
    // NFT Bronze (Tier 1) - Prix en USDC avec 6 décimales
    _createTier(1, NFTTier({
        price: 120 * 10**6, // 120 USDC (avec 6 décimales)
        supply: 1000,
        minted: 0,
        baseURI: "https://olive-quick-dolphin-266.mypinata.cloud/ipfs/bafybeihd4hzlhs755vrk3ffrdaxaru34hyf7ksuvzycbtk5vqjlx72cyzi/",
        active: true,
        name: "Vaelith de Seve",
    ...
    _createTier(2, NFTTier({
        price: 250 * 10**6, // 250 USDC (avec 6 décimales)
        supply: 500,
        minted: 0,
        baseURI: "https://olive-quick-dolphin-266.mypinata.cloud/ipfs/bafybeihd4hzlhs755vrk3ffrdaxaru34hyf7ksuvzycbtk5vqjlx72cyzi/",
        active: true,
        name: "Sylwen des Brumes",
    ...
    _createTier(3, NFTTier({
        price: 500 * 10**6, // 500 USDC (avec 6 décimales)
        supply: 200,
        minted: 0,
        baseURI: "https://olive-quick-dolphin-266.mypinata.cloud/ipfs/bafybeihd4hzlhs755vrk3ffrdaxaru34hyf7ksuvzycbtk5vqjlx72cyzi/",
        active: true,
        name: "Nymeriel de Lune",
    ...
    _createTier(4, NFTTier({
        price: 1000 * 10**6, // 1000 USDC (avec 6 décimales)
        supply: 50,
        minted: 0,
        baseURI: "https://olive-quick-dolphin-266.mypinata.cloud/ipfs/bafybeihd4hzlhs755vrk3ffrdaxaru34hyf7ksuvzycbtk5vqjlx72cyzi/",
        active: true,
        name: "Faeloria la Radieuse",
    ...
    uint256 usdcPrice = nftTiers[tier].price; // Déjà en bonnes décimales !
    ...
    totalRevenue += usdcPrice;
    tierRevenue[tier] += usdcPrice;
    
    // Minter le NFT
    _mintNFT(msg.sender, tier, false);
    
    // Emettre les evenements
    emit NFTPurchased(msg.sender, _tokenIdCounter, tier, usdcPrice);
    emit RevenueGenerated(tier, usdcPrice, TREASURY_WALLET);
}
    ...
    function claimFidelityNFT(address fidelUser) external onlyOwner {
        uint256 fidelityTier = 5; // Tier 5 pour la fidelite
        
        require(nftTiers[fidelityTier].active, "Tier fidelite non actif");
        require(nftTiers[fidelityTier].minted < nftTiers[fidelityTier].supply, "Supply fidelite epuisee");
        require(!ownerHasTier[fidelUser][fidelityTier], "NFT Fidelite deja possede");
        
        _mintNFT(fidelUser, fidelityTier, true);
        
        emit FidelityNFTClaimed(fidelUser, _tokenIdCounter);
    }

    // FONCTION: Reclamation autonome pour les utilisateurs eligibles
    function claimMyFidelityNFT() external {
        uint256 fidelityTier = 5;
        
        require(fidelityEligible[msg.sender], "Non eligible pour NFT fidelite");
        require(nftTiers[fidelityTier].active, "Tier fidelite non actif");
        require(nftTiers[fidelityTier].minted < nftTiers[fidelityTier].supply, "Supply fidelite epuisee");
        require(!ownerHasTier[msg.sender][fidelityTier], "NFT Fidelite deja possede");
        
        _mintNFT(msg.sender, fidelityTier, true);
        fidelityEligible[msg.sender] = false; // Une seule reclamation
        
        emit FidelityNFTClaimed(msg.sender, _tokenIdCounter);
    }

    // FONCTIONS: Gestion de l'eligibilite fidelite
    function setFidelityEligible(address user, bool eligible) external onlyOwner {
        fidelityEligible[user] = eligible;
        emit FidelityEligibilitySet(user, eligible);
    }

    function setMultipleFidelityEligible(address[] memory users, bool eligible) external onlyOwner {
        for (uint i = 0; i < users.length; i++) {
            fidelityEligible[users[i]] = eligible;
            emit FidelityEligibilitySet(users[i], eligible);
        }
    }

    // FONCTION: Verifier l'eligibilite fidelite
    function checkFidelityEligibility(address user) external view returns (
        bool eligible,
        bool alreadyClaimed,
        bool tierActive,
        uint256 remaining
        ...
        return (
            fidelityEligible[user],
            ownerHasTier[user][fidelityTier],
            nftTiers[fidelityTier].active,
            nftTiers[fidelityTier].supply - nftTiers[fidelityTier].minted
        );
    }
    ...
    function _afterTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            ...
            // Mettre à jour _ownedTokens pour l'ancien proprietaire
            ...
            // Mettre à jour _ownedTokens pour le nouveau proprietaire
            _ownedTokens[to].push(tokenId);
            _ownedTokensIndex[tokenId] = _ownedTokens[to].length - 1;
