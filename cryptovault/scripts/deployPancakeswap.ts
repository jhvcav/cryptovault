...
import { ethers, run } from "hardhat"; // Importez run directement de hardhat
dotenv.config();
...
async function main() {
  ...
  const CakeStakingStrategyUSDC = await ethers.getContractFactory("CakeStakingStrategy");
  ...
  const CakeStakingStrategyUSDT = await ethers.getContractFactory("CakeStakingStrategy");
  ...
  if (process.env.NETWORK !== "hardhat" && process.env.NETWORK !== "localhost") {
    ...
    try {
      await run("verify:verify", { // Utilisez run au lieu de ethers.run
