...
exports.handler = async (event, context) => {
  console.log('🔍 Function appelée - Method:', event.httpMethod);
  ...
  try {
    ...
    if (memberData && memberData.username && memberData.email) {
      ...
      // Si SMTP est configuré, essayer d'envoyer l'email
      if (hasSmtp) {
        try {
          const nodemailer = require('nodemailer');
          ...
          // Contenu de l'email
          const mailOptions = {
            from: `"CryptocaVault" <${process.env.SMTP_USER}>`,
            to: process.env.SMTP_USER,
            subject: '🌟 Nouvelle inscription communauté RMR',
            html: `
              <h2>🌟 Nouvelle inscription communauté RMR</h2>
              <p><strong>👤 Utilisateur:</strong> ${memberData.username}</p>
...
Utilisateur: ${memberData.username}
Email: ${memberData.email}
...
Date: ${new Date().toLocaleString('fr-FR')}
          ...
          return {
            ...
            body: JSON.stringify({
              success: true,
              message: 'Inscription loggée et email envoyé',
              messageId: info.messageId,
              memberData: {
                username: memberData.username,
                email: memberData.email,
    ...
    // Réponse de succès (avec ou sans email)
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({
        success: true,
        message: 'Inscription loggée avec succès',
        memberData: memberData ? {
          username: memberData.username,
          email: memberData.email,
          phone: memberData.phone || 'Non renseigné',
